...
const AdminDashboard = () => {
  ...
  const COLORS = ['hsl(42, 70%, 55%)', 'hsl(42, 50%, 40%)', 'hsl(152, 55%, 42%)', 'hsl(220, 12%, 40%)'];
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(stat => (
          <Card key={stat.title} className="rounded-2xl border-border bg-card">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="rounded-2xl border-border bg-card">
          <CardHeader>
            <CardTitle className="font-display text-lg text-foreground">Receita Mensal</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <AreaChart data={monthlySales}>
                <defs>
                  <linearGradient id="colorReceita" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="hsl(42, 70%, 55%)" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="hsl(42, 70%, 55%)" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(220, 10%, 18%)" />
                <XAxis dataKey="month" tick={{ fontSize: 12, fill: 'hsl(220, 8%, 55%)' }} />
                <YAxis tick={{ fontSize: 12, fill: 'hsl(220, 8%, 55%)' }} />
                <Tooltip contentStyle={{ background: 'hsl(220, 14%, 11%)', border: '1px solid hsl(220, 10%, 18%)', borderRadius: '8px', color: 'hsl(40, 15%, 90%)' }} />
                <Area type="monotone" dataKey="receita" stroke="hsl(42, 70%, 55%)" fill="url(#colorReceita)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-border bg-card">
          <CardHeader>
            <CardTitle className="font-display text-lg text-foreground">Livros por Gênero</CardTitle>
          </CardHeader>
      ...
      <Card className="rounded-2xl border-border bg-card">
        <CardHeader>
          <CardTitle className="font-display text-lg text-foreground">Vendas Recentes</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <tbody>
                {sales.slice(-5).reverse().map(sale => (
                  <tr key={sale.id} className="border-b border-border last:border-0 hover:bg-muted/50 transition-colors">
                    <td className="py-3 px-3 font-medium text-foreground">#{sale.id}</td>
                    <td className="py-3 px-3 text-foreground">{sale.customer?.name}</td>
                    <td className="py-3 px-3 text-muted-foreground">{new Date(sale.date_sale).toLocaleDateString('pt-BR')}</td>
