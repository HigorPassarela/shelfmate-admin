            ...
            <div key={author.id} className="rounded-2xl border border-border bg-card p-6 md:p-8 transition-all hover:border-primary/20">
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                  ...
                  {author.biography && <p className="text-muted-foreground leading-relaxed mb-5">{author.biography}</p>}
                  <div>
                    <h4 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">Obras no acervo</h4>
                    <div className="flex flex-wrap gap-2">
                      {authorBooks.map(b => (
                        <span key={b.id} className="inline-flex items-center gap-1.5 rounded-full bg-secondary px-3.5 py-1.5 text-sm font-medium text-secondary-foreground">
                          {b.title}
